...
環境変数 `OWNER_EMAILS` に登録したメールアドレスのアカウントは、**サブスク未加入でも全機能を無制限**で利用できる（課金なし・生成回数カウントなし）。

```
OWNER_EMAILS=team.yamazaki.2009@gmail.com
```

- 複数人はカンマ区切り: `OWNER_EMAILS=a@example.com,b@example.com`
- 対象ユーザーも通常どおり**アカウント登録＋メール認証は必要**（ログイン後の課金チェックだけがスキップされる）
- Stripe に顧客データは作られない
- 解除するには環境変数からメールを削除して再デプロイ
