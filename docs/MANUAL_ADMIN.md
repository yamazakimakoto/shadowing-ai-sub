...
| 項目 | 値 |
|------|-----|
| 月額料金 | Stripe Dashboard で設定（`STRIPE_PRICE_ID`）|
| 月次生成回数 | 30回（`subscriptions.gen_max` デフォルト）|
| TTS（音声合成）| 無制限 |
| 採点（Whisper）| 無制限 |
| テキスト保存 | 無制限 |

### コスト目安（OpenAI / Anthropic）

| API | 単価 | 1ユーザー1ヶ月想定 |
|-----|------|-------------------|
| Claude（生成）| ~¥0.5 / 回 | 30回 = ¥15 |
| Claude（解説）| ~¥1 / 回 | 30回 = ¥30 |
| OpenAI TTS | $0.015 / 1,000文字 | キャッシュあり、~¥30〜50 |
| OpenAI Whisper | $0.006 / 分 | 100回練習 = ¥100 |

**1ユーザーあたり月額原価目安: 約¥150〜200**
